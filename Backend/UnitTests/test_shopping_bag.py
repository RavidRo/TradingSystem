...
def products_stubs_shopping_bag_not_existing_prod():
    products_to_quantities = {'1': (ProductStub('product1'), 2), '2': (ProductStub('product2'), 1),
                              '3': (ProductStub('product3'), 1), '4': (ProductStub('product4'), 4)}
    return products_to_quantities
...
def test_buy_products_valid(shopping_bag: ShoppingBag, user_stub: UserStub, products_stubs_store: dict,
...
@patch.multiple(StoreStub, apply_discounts=MagicMock(return_value=10))
def test_buy_products_not_existing(shopping_bag: ShoppingBag, user_stub: UserStub, products_stubs_store: dict,
                                   products_stubs_shopping_bag_not_existing_prod: dict):
    with patch.dict(shopping_bag.store.products_to_quantities, products_stubs_store):
        with patch.dict(shopping_bag.products_to_quantity, products_stubs_shopping_bag_not_existing_prod):
            result = shopping_bag.buy_products(user_stub)
            assert result.success == False
...
@patch.multiple(StoreStub, apply_discounts=MagicMock(return_value=10))
def test_buy_products_missing_quantity(shopping_bag: ShoppingBag, user_stub: UserStub, products_stubs_store: dict,
                                       products_stubs_shopping_bag_missing_prod: dict):
    with patch.dict(shopping_bag.store.products_to_quantities, products_stubs_store):
        with patch.dict(shopping_bag.products_to_quantity, products_stubs_shopping_bag_missing_prod):
            result = shopping_bag.buy_products(user_stub)
            assert result.success == False
            assert shopping_bag.store.products_to_quantities == products_stubs_store
...
# * ====================================================
def test_change_product_quantity_valid(shopping_bag: ShoppingBag, products_stubs_shopping_bag: dict):
    with patch.dict(shopping_bag.products_to_quantity, products_stubs_shopping_bag):
...
def test_change_product_quantity_invalid_amount(shopping_bag: ShoppingBag, products_stubs_shopping_bag: dict):
    with patch.dict(shopping_bag.products_to_quantity, products_stubs_shopping_bag):
        result = shopping_bag.change_product_quantity('1', -7)
        assert result.success == False
        assert shopping_bag.products_to_quantity.get('1')[1] == 2
...
def test_change_product_quantity_not_existing_product(shopping_bag: ShoppingBag, products_stubs_shopping_bag: dict):
    with patch.dict(shopping_bag.products_to_quantity, products_stubs_shopping_bag):
