...
def simple_product_false_conditional_discount(product_ids):
    return {'discount_type': 'simple',
            'percentage': 75.0,
...
def simple_category_true_conditional_discount(product_ids):
    return {'discount_type': 'simple',
            'percentage': 50.0,
            'condition': {'context': {'obj': 'product', 'identifier': product_ids[0]}, 'operator': 'less-than',
                          'target': 8},
...
def test_purchase_success():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id
...
def fail_purchase_payment_case():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id
...
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_purchase_fail_false_payment():
    fail_purchase_payment_case()
...
def test_purchase_fail_false_deliver():
    fail_purchase_payment_case()
...
@patch.multiple(CashingAdapter, pay=MagicMock(return_value=Response(False)))
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(False)))
def test_purchase_fail_false_payment_false_deliver():
    fail_purchase_payment_case()
...
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_purchase_fail_crash_payment():
    fail_purchase_payment_case()
...
def test_purchase_fail_crash_deliver():
    fail_purchase_payment_case()


@patch.multiple(CashingAdapter, pay=MagicMock(return_value=Response(True)))
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_purchase_rule_fail_blocking_rule():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id

    system.add_purchase_rule(cookie, store_id, simple_product_rule(product_ids), "simple", "1")
    for product_id in product_ids:
        system.save_product_in_cart(cookie, store_id, product_id, 2)
...
@patch.multiple(CashingAdapter, pay=MagicMock(return_value=Response(True)))
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_simple_conditional_discount_true_cond():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id
    ...
    system.add_discount(cookie, store_id, simple_product_true_conditional_discount(product_ids), "1", "simple")
...
@patch.multiple(CashingAdapter, pay=MagicMock(return_value=Response(True)))
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_simple_conditional_discount_false_cond():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id

    for product_id in product_ids:
        system.save_product_in_cart(cookie, store_id, product_id, 2)
    system.add_discount(cookie, store_id, simple_product_false_conditional_discount(product_ids), "1", "simple")
    price_res = system.purchase_cart(cookie, 12)
    res = system.send_payment(cookie, "", "")
    assert res.succeeded() and len(system.get_cart_details(cookie).object.bags) == 0 \
           and all(list(
...
@patch.multiple(CashingAdapter, pay=MagicMock(return_value=Response(True)))
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_and_conditional_discount_false():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id
    ...
    system.add_discount(cookie, store_id, simple_product_true_conditional_discount(product_ids), "2", "simple")

    system.add_discount(cookie, store_id, simple_category_false_conditional_discount(product_ids), "2", "simple")
    price_res = system.purchase_cart(cookie, 12)
    res = system.send_payment(cookie, "", "")
    assert res.succeeded() and len(system.get_cart_details(cookie).object.bags) == 0 \
           and all(list(
...
@patch.multiple(CashingAdapter, pay=MagicMock(return_value=Response(True)))
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_and_conditional_discount_true():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id
    ...
    system.add_discount(cookie, store_id, simple_category_true_conditional_discount(product_ids), "2", "simple")
    price_res = system.purchase_cart(cookie, 12)
    res = system.send_payment(cookie, "", "")
    assert res.succeeded() and len(system.get_cart_details(cookie).object.bags) == 0 \
           and all(list(
...
@patch.multiple(CashingAdapter, pay=MagicMock(return_value=Response(True)))
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_or_conditional_discount_true():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id
...
@patch.multiple(CashingAdapter, pay=MagicMock(return_value=Response(True)))
@patch.multiple(SupplyAdapter, deliver=MagicMock(return_value=Response(True)))
def test_or_conditional_discount_false():
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id
    ...
    system.add_discount(cookie, store_id, simple_product_false_conditional_discount(product_ids), "2", "simple")
    price_res = system.purchase_cart(cookie, 12)
    res = system.send_payment(cookie, "", "")
...
def test_try_paying_first_time_fail_second_time_timer_over():
    import time
    system, cookie, store, product_ids = initialization(auto_id())
    store_id = store.id
