...
def test_add_simple_purchase_rule_success(store: Store):
    ...
    added_rule = store.parse_purchase_policiy()
    rule_details['id'] = '2'
...
def test_add_complex_purchase_rule_and_simple_child_success(store: Store, complex_rule_details,
                                                            simple_rule_details_upper):
    result_complex, result_simple = add_complex_simple_rules(complex_rule_details, simple_rule_details_upper, store)
    added_rules = store.parse_purchase_policiy()
...
def test_add_conditional_rule_success(store: Store):
    ...
    result_test_clause = store.add_purchase_rule(test_clause_details, 'simple', test_then_clause_parent_id,
                                                 clause="test")
    result_then_clause = store.add_purchase_rule(then_clause_details, 'simple', test_then_clause_parent_id,
                                                 clause="then")
    added_rules = store.parse_purchase_policiy()
...
def test_add_rule_with_leaf_as_parent_fail(store: Store, complex_rule_details, simple_rule_details_upper):
...
def test_remove_simple_purchase_rule_success(store: Store, complex_rule_details, simple_rule_details_upper):
    add_complex_simple_rules(complex_rule_details, simple_rule_details_upper, store)
    rule_id = '3'
    response_remove = store.remove_purchase_rule(rule_id)
    after_remove_rules = store.parse_purchase_policiy()
    complex_rule_details['id'] = '2'
...
def test_remove_complex_and_simple_purchase_rules_success(store: Store, complex_rule_details, simple_rule_details_upper):
    add_complex_simple_rules(complex_rule_details, simple_rule_details_upper, store)
    rule_id = '2'
    response_remove = store.remove_purchase_rule(rule_id)
    after_remove_rules = store.parse_purchase_policiy()
...
def test_edit_simple_purchase_rule_success(store: Store, complex_rule_details, simple_rule_details_upper,
                                           edited_simple_rule_details):
    add_complex_simple_rules(complex_rule_details, simple_rule_details_upper, store)
    edited_rule_id = '3'
...
def test_edit_complex_purchase_rule_success(store: Store, complex_rule_details, simple_rule_details_upper,
                                            edited_complex_rule_details):
    add_complex_simple_rules(complex_rule_details, simple_rule_details_upper, store)
    edited_rule_id = '2'
...
def test_edit_not_existing_rule(store: Store, edited_simple_rule_details):
    ...
    after_edit_rules = store.parse_purchase_policiy()
    expected_rule = {'id': '1', 'operator': 'and', 'children': []}
...
def test_move_purchase_rule_sucess(store: Store, complex_rule_details, simple_rule_details_upper):
    ...
    moved_id = '3'
    before_move_rules = store.parse_purchase_policiy()
    result_move = store.move_purchase_rule(moved_id, another_complex_id)
    after_move_rules = store.parse_purchase_policiy()
    another_complex_details['children'] = [simple_rule_details_upper]
    another_complex_details['id'] = '4'
    complex_rule_details['id'] = '2'
    complex_rule_details['children'] = []
    simple_rule_details_upper['id'] = '3'
    expected_rule = {'id': '1', 'operator': 'and', 'children': [complex_rule_details, another_complex_details]}
...
#region tests- age > 18
def test_apply_simple_discount_success(store: Store, simple_rule_details_upper, products_to_quantities):
...
def test_apply_simple_discount_fail(store: Store, simple_rule_details_upper, products_to_quantities):
    store.add_purchase_rule(simple_rule_details_upper, 'simple', '1')
    user_age = 17
    response = store.check_purchase(products_to_quantities, user_age)
...
def test_apply_and_complex_age_rule_success(store: Store, simple_rule_details_upper, simple_rule_details_down,products_to_quantities):
...
def test_apply_and_complex_age_rule_fail(store: Store, simple_rule_details_upper, simple_rule_details_down,products_to_quantities):
    store.add_purchase_rule({'operator': 'and'}, 'complex', '1')
    store.add_purchase_rule(simple_rule_details_upper, 'simple', '2')
    store.add_purchase_rule(simple_rule_details_down, 'simple', '2')
    user_age = 26
...
def test_apply_condition_discount_success(store: Store, products_to_quantities):
    complex_rule_details = {'operator': 'conditional'}
    test_clause_details = {'context': {'obj': 'product', 'identifier': '1'}, 'operator': 'less-than', 'target': 10}
    then_clause_details = {'context': {'obj': 'category', 'identifier': 'milk products'}, 'operator': 'equals',
                           'target': 11}
...
@patch.multiple(ProductStub, get_category=MagicMock(return_value='milk products'))
def test_apply_condition_discount_success_irrelevant_test_clause(store: Store, products_to_quantities):
    complex_rule_details = {'operator': 'conditional'}
    test_clause_details = {'context': {'obj': 'product', 'identifier': '1'}, 'operator': 'less-than', 'target': 8}
    then_clause_details = {'context': {'obj': 'category', 'identifier': 'milk products'}, 'operator': 'equals',
                           'target': 9}
...
@patch.multiple(ProductStub, get_category=MagicMock(return_value='milk products'))
def test_very_complex_success(store: Store, simple_rule_details_upper, simple_rule_details_down, products_to_quantities):
    store.add_purchase_rule({'operator': 'and'}, 'complex', '1')
    ...
    simple_rule_under_and = {'context': {'obj': 'category', 'identifier': 'milk products'}, 'operator': 'equals',
                           'target': 11}
...
# @patch.multiple(ProductStub, get_category=MagicMock(return_value='milk products'))
# def test_very_complex_fail_beacuse_of_rule_under_and(store: Store, simple_rule_details_upper, simple_rule_details_down, products_to_quantities):
#     store.add_purchase_rule({'operator': 'and'}, 'complex', '1')
#     store.add_purchase_rule({'operator': 'or'}, 'complex', '2')
#     store.add_purchase_rule({'operator': 'or'}, 'complex', '2')
#     simple_rule_result_true = {'context': {'obj': 'product', 'identifier': '1'}, 'operator': 'less-than', 'target': 10}
