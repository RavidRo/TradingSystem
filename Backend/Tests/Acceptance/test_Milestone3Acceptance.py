import threading
from Backend.Tests.Acceptance.test_TradingSystem import (
    username_number,
    store_number,
    product_number,
)
...
def test_buy_offer_success():
    cookie, username, password, store_name, store_id = _initialize_info(
        _generate_username(), "pass", _generate_store_name()
    )
    product_id, product_name, category, price, quantity = _create_product(
        cookie, store_id, _generate_product_name(), "cat", 1.2, 10
    )
...
def test_declare_then_counter_offer_twice_success():
    ...
    product_id, product_name, category, price, quantity = _create_product(
        cookie, store_id, _generate_product_name(), "cat", 10.0, 10
    )
