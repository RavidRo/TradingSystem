...
def __initialize_info(
    username: str, password: str, store_name: str = None
) -> tuple[str, str, str, str, str]:
    store_id = ""
    cookie = __system.enter_system()
    __system.register(cookie, username, password)
    __system.login(cookie, username, password)
    if store_name:
        store_id = __system.create_store(cookie, store_name).object
    return cookie, username, password, store_name, store_id


def __create_product(cookie: str, store_id: str, product_name: str, price: float, quantity: int) -> tuple[str, str, float, int]:
    product_id = __system.create_product(cookie, store_id, product_name, price, quantity).object
...
def test_register_success():
    ...
    password = "aaa"
    cookie = __system.enter_system()
    assert __system.register(cookie, new_username, password).succeeded()
...
def test_register_used_username_fail():
    ...
    password = "aaa"
    cookie = __system.enter_system()
    __system.register(cookie, existing_username, password)
    assert not __system.register(cookie, existing_username, password).succeeded()
...
def test_login_success():
    ...
    password = "aaa"
    cookie = __system.enter_system()
    __system.register(cookie, new_username, password)
    assert __system.login(cookie, new_username, password).succeeded()
...
def test_login_wrong_username_fail():
    ...
    cookie = __system.enter_system()
    __system.register(cookie, new_username, password)
    assert not __system.login(cookie, wrong_username, password).succeeded()
...
def test_login_wrong_password_fail():
    ...
    cookie = __system.enter_system()
    __system.register(cookie, new_username, password)
    assert not __system.login(cookie, new_username, wrong_password).succeeded()
...
def test_open_store_success():
    cookie, username, password, _, _ = __initialize_info(__generate_username(), "aaa")
    store_name = __generate_store_name()
    assert __system.create_store(cookie, store_name).succeeded()
...
#     assert not __system.create_store(cookie, store_name).succeeded()
...
#     cookie, username, password, _ = __initialize_info(__generate_username(), "aaa")
#     assert not __system.get_stores_details().succeeded()  # an empty list evaluates to false
...
def test_add_new_product_success():
    ...
    price = 5.50
    quantity = 10
    response = __system.create_product(cookie, store_id, product_name, price, quantity)
...
def test_add_new_product_negative_quantity_fail():
    ...
    price = 5.50
    quantity = -10
    assert not __system.create_product(cookie, store_id, product_name, price, quantity).succeeded()
...
def test_remove_product_success():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    assert __system.remove_product_from_store(cookie, store_id, product_id).succeeded()
...
def test_remove_product_wrong_product_fail():
    ...
    wrong_product = "cofee"
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    assert not __system.remove_product_from_store(cookie, store_id, wrong_product).succeeded()
...
def test_change_product_quantity_success():
    ...
    new_quantity = 15
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    response = __system.change_product_quantity_in_store(
...
def test_change_product_quantity_negative_quantity_fail():
    ...
    new_quantity = -15
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    assert not __system.change_product_quantity_in_store(
        cookie, store_id, product_id, new_quantity
    ).succeeded()
...
def test_edit_product_details_success():
    ...
    new_price = 6.0
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    assert __system.edit_product_details(
...
def test_edit_product_details_wrong_product_fail():
    ...
    new_price = 6.0
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    assert not __system.edit_product_details(
...
def test_product_search_no_args_success():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    response = __system.search_products(product_name=product_name)
    assert (
        response.succeeded()
...
def test_product_search_args_success():
    ...
    min_price = 5.0
    max_price = 6.0
    response = __system.search_products(product_name, min_price=min_price, max_price=max_price)
    assert (
        response.succeeded()
        and len(list(filter(lambda product: product.name == product_name, response.object.values))) == 1
    )
...
#     price = 5.50
#     quantity = 10
#     __system.create_product(cookie, store_name, product_name, price, quantity)
#     response = __system.search_products(wrong_product)
...
#     max_price = 6.0
#     __system.create_product(cookie, store_name, product_name, price, quantity)
#     response = __system.search_products(wrong_product, min_price=min_price, max_price=max_price)
#     assert not response.succeeded()
# assumed empty list means failure
...
def test_products_by_store_success():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    response = __system.get_products_by_store(store_id)
    assert (
        response.succeeded()
...
def test_products_by_store_wrong_store_fail():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    wrong_store = "starbux"
    response = __system.get_products_by_store(wrong_store)
    assert not response.succeeded()
...
def test_add_to_cart_success():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    assert __system.save_product_in_cart(cookie, store_id, product_id, 1).succeeded()
...
def test_add_to_cart_wrong_product_fail():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    wrong_product = "cofee"
    assert not __system.save_product_in_cart(cookie, store_id, wrong_product, 1).succeeded()
...
def test_add_to_cart_wrong_store_fail():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    wrong_store = "starbux"
    assert not __system.save_product_in_cart(cookie, wrong_store, product_id, 1).succeeded()
...
def test_add_to_cart_quantity_too_high_fail():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    assert not __system.save_product_in_cart(cookie, store_id, product_id, 11).succeeded()
...
def test_visit_cart_success():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    response = __system.get_cart_details(cookie)
    assert (
        response.succeeded()
...
#         __generate_username(), "aaa", __generate_store_name()
#     )
#     assert not __system.get_cart_details(cookie).succeeded()
#   assumed empty list means failure
...
def test_change_amount_in_cart_success():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    response = __system.change_product_quantity_in_cart(cookie, store_id, product_id, 2)
    assert (
            response.succeeded()
            and __system.get_cart_details(cookie).object.bags[0].product_ids_to_quantities[product_id] == 2
    ), response.get_msg()
...
def test_change_amount_in_cart_wrong_product_fail():
    ...
    wrong_product = "cofee"
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    assert not __system.change_product_quantity_in_cart(
...
def test_remove_product_from_cart_success():
    ...
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    assert (
        __system.remove_product_from_cart(cookie, store_id, product_id).succeeded()
    )
...
def test_remove_product_from_cart_wrong_product_fail():
    ...
    wrong_product = "cofee"
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    assert not __system.remove_product_from_cart(cookie, store_id, wrong_product).succeeded()
...
def test_remove_product_from_cart_wrong_store_fail():
    ...
    wrong_store = "starbux"
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    assert not __system.remove_product_from_cart(cookie, wrong_store, product_id).succeeded()
...
def test_purchase_cart_success():
    ...
    product_id, product_name, price, quantity = __create_product(cookie, store_id, __generate_product_name(), 5.50, 10)
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    response = __system.purchase_cart(cookie)
    assert (
            response.succeeded()
            and __system.get_store(store_id).object.ids_to_quantities[product_id] == 9
            and __system.get_cart_details(cookie).succeeded()
...
def test_purchase_cart_no_items_fail():
    ...
    assert not __system.purchase_cart(cookie).succeeded()
...
def test_send_payment_success():
    ...
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    __system.purchase_cart(cookie)
    response = __system.send_payment(cookie, {}, {})
    assert (
            response.succeeded()
            and __system.get_store(store_id).object.ids_to_quantities[product_id] == 9
    ), response.get_msg()
...
def test_send_payment_before_purchase_cart_fail():
    ...
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    assert (
            not __system.send_payment(cookie, {}, {}).succeeded()
            and __system.get_store(store_id).object.ids_to_quantities[product_id] == 10
            and __system.get_cart_details(cookie).succeeded()
    )
...
def test_get_purchase_history_success():
    ...
    __system.save_product_in_cart(cookie, store_id, product_id, 1)
    __system.purchase_cart(cookie)
    __system.send_payment(cookie, {}, {})
    response = __system.get_purchase_history(cookie)
    assert (
        response.succeeded()
        and len(response.object.values) == 1
...
def test_appoint_store_owner_success():
    ...
    new_owner_cookie, new_owner_username, new_owner_password, _, _ = __initialize_info(
        __generate_username(), "bbb"
    )
    response = __system.appoint_owner(cookie, store_id, new_owner_username)
    assert response.succeeded(), response.get_msg()
...
def test_appoint_store_owner_chain_success():
    ...
    last_owner_cookie, last_owner_username, last_owner_password, _, _ = __initialize_info(
        __generate_username(), "ccc"
    )
    __system.appoint_owner(cookie, store_id, new_owner_username)
    assert __system.appoint_owner(new_owner_cookie, store_id, last_owner_username).succeeded()
...
def test_appoint_store_owner_wrong_name_fail():
    ...
    assert not __system.appoint_owner(cookie, store_id, wrong_name).succeeded()
...
def test_appoint_store_owner_wrong_store_fail():
    ...
    wrong_store = "starbux"
    assert not __system.appoint_owner(cookie, wrong_store, new_owner_username).succeeded()
...
def test_appoint_store_owner_direct_circular_appointment_fail():
    ...
    __system.appoint_owner(cookie, store_id, new_owner_username)
    assert not __system.appoint_owner(new_owner_cookie, store_id, username).succeeded()
...
def test_appoint_store_owner_circular_fail():
    ...
    __system.appoint_owner(cookie, store_id, new_owner_username)
    __system.appoint_owner(new_owner_cookie, store_id, last_owner_username)
    assert not __system.appoint_owner(last_owner_cookie, store_id, username).succeeded()
...
def test_appoint_store_manager_success():
    ...
    assert __system.appoint_manager(cookie, store_id, new_manager_username).succeeded()
...
#         __generate_username(), "ccc"
#     )
#     __system.appoint_manager(cookie, store_name, new_manager_username)
#     assert __system.appoint_manager(new_manager_cookie, store_name, last_manager_username).succeeded()
# tested elsewhere
...
def test_appoint_store_owner_manager_chain_success():
    ...
    __system.appoint_owner(cookie, store_id, new_owner_username)
    assert __system.appoint_manager(new_owner_cookie, store_id, last_manager_username).succeeded()
...
def test_appoint_store_manager_wrong_name_fail():
    ...
    wrong_name = "Ravit Ron"
    assert not __system.appoint_manager(cookie, store_id, wrong_name).succeeded()
...
def test_appoint_store_manager_wrong_store_fail():
    ...
    wrong_store = "starbux"
    assert not __system.appoint_manager(cookie, wrong_store, new_manager_username).succeeded()
...
def test_appoint_store_manager_direct_circular_appointment_fail():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    assert not __system.appoint_manager(new_manager_cookie, store_id, username).succeeded()
...
#     )
#     __system.appoint_manager(cookie, store_id, new_manager_username)
#     __system.appoint_manager(new_manager_cookie, store_id, last_manager_username)
#     assert not __system.appoint_manager(last_manager_cookie, store_name, username).succeeded()
# tested elsewhere
...
def test_appoint_store_manager_owner_chain_fail():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    assert not __system.appoint_owner(new_manager_cookie, store_id, last_owner_username).succeeded()
...
def test_add_responsibility_success():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    response = __system.add_manager_permission(
...
def test_remove_responsibility_success():
    ...
    assert __system.remove_manager_permission(
        cookie, store_id, new_manager_username, new_responsibility
    ).succeeded()
...
def test_default_permissions_success():
    ...
    for responsibility in other_permissions:
        assert __system.add_manager_permission(
...
def test_get_appointment_permission_success():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    response = __system.get_store_appointments(new_manager_cookie, store_id)
    assert response.succeeded(), response.get_msg()
...
def test_get_history_permission_success():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    __system.add_manager_permission(cookie, store_id, new_manager_username, "get_history")
    assert __system.get_store_purchase_history(new_manager_cookie, store_id).succeeded()
...
def test_appoint_manager_permission_success():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    __system.add_manager_permission(cookie, store_id, new_manager_username, "appoint_manager")
    assert __system.appoint_manager(new_manager_cookie, store_id, last_manager_username).succeeded()
...
def test_remove_manager_permission_fail():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    __system.appoint_manager(cookie, store_id, last_manager_username)
    __system.add_manager_permission(cookie, store_id, new_manager_username, "remove_manager")
    assert not __system.remove_appointment(
...
def test_remove_manager_permission_success():
    ...
    __system.appoint_manager(new_manager_cookie, store_id, last_manager_username)
    __system.add_manager_permission(cookie, store_id, new_manager_username, "remove_manager")
    response = __system.remove_appointment(new_manager_cookie, store_id, last_manager_username)
    assert response.succeeded(), response.get_msg()
...
def test_manage_products_permission_success():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    __system.add_manager_permission(cookie, store_id, new_manager_username, "manage_products")
    product_name = __generate_product_name()
    price = 5.50
    quantity = 10
    response = __system.create_product(new_manager_cookie, store_id, product_name, price, quantity)
    assert response.succeeded(), response.get_msg()
...
def test_get_appointment_no_permission_fail():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    __system.remove_manager_permission(cookie, store_id, new_manager_username, "get_appointments")
    assert not __system.get_store_appointments(new_manager_cookie, store_id).succeeded()
...
def test_get_history_no_permission_fail():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    assert not __system.get_store_purchase_history(new_manager_cookie, store_id).succeeded()
...
def test_appoint_manager_no_permission_fail():
    ...
    __system.appoint_manager(cookie, store_id, new_manager_username)
    assert not __system.appoint_manager(
        new_manager_cookie, store_id, last_manager_username
    ).succeeded()
...
def test_manage_products_no_permission_fail():
    ...
    price = 5.50
    quantity = 10
    assert not __system.create_product(
...
def test_dismiss_owner_success():
    ...
    __system.appoint_owner(cookie, store_id, new_owner_username)
    response = __system.remove_appointment(cookie, store_id, new_owner_username)
...
def test_dismiss_owner_wrong_name_fail():
    ...
    wrong_name = "Ravit Ron"
    __system.appoint_owner(cookie, store_id, new_owner_username)
    assert not __system.remove_appointment(cookie, store_id, wrong_name).succeeded()
...
def test_dismiss_owner_wrong_store_fail():
    ...
    wrong_store = "starbux"
    __system.appoint_owner(cookie, store_id, new_owner_username)
    assert not __system.remove_appointment(cookie, wrong_store, new_owner_username).succeeded()
...
def test_dismiss_owner_appointing_fail():
    ...
    __system.appoint_owner(cookie, store_id, new_owner_username)
    __system.remove_appointment(cookie, store_id, new_owner_username)
    assert not __system.appoint_manager(
...
def test_dismiss_owner_chain_appointing_fail():
    ...
    final_manager_cookie, final_manager_username, final_manager_password, _, _ = __initialize_info(
...
def test_get_store_personnel_success():
    ...
    response = __system.get_store_appointments(cookie, store_id)
    assert (
        response.succeeded()
...
def test_get_store_personnel_wrong_store_name_fail():
    ...
    wrong_store = "starbux"
    assert not __system.get_store_appointments(cookie, wrong_store).succeeded()
...
def test_get_store_purchase_history_success():
    ...
    __system.purchase_cart(cookie)
    __system.send_payment(cookie, {}, {})
    response = __system.get_store_purchase_history(cookie, store_id)
    assert (
        response.succeeded()
        and len(response.object.values) == 1
        and response.object.values[0].product_names[0] == product_name
...
#     quantity = 10
#     __system.create_product(cookie, store_name, product_name, price, quantity)
#     response = __system.get_store_purchase_history(cookie, store_name)
#     assert response.succeeded()
# assumed empty list means failure
...
def _get_admin() -> str:
    admin_cookie = __system.enter_system()
    with open("config.json",  "r") as read_file:
        data = json.load(read_file)
        __system.login(admin_cookie, data["admins"][0], data["admin-password"])
    return admin_cookie
...
def test_admin_get_store_purchase_history_success():
    ...
    __system.purchase_cart(cookie)
    __system.send_payment(cookie, {}, {})
    response = __system.get_any_store_purchase_history(admin_cookie, store_id)
    assert (
        response.succeeded()
        and len(response.object.values) == 1
        and response.object.values[0].product_names[0] == product_name
...
def test_admin_get_user_purchase_history_success():
    ...
    __system.purchase_cart(cookie)
    __system.send_payment(cookie, {}, {})
    response = __system.get_user_purchase_history(admin_cookie, username)
    assert response.succeeded()
