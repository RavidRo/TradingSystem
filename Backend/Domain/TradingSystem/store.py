...
class Store:
    ...
    def __init__(self, store_name: str):
        ...
        self.__products_to_quantities: dict[str, tuple[Product, int]] = {}
        ...
        self.__products_lock = ReadWriteLock()
        self.__history_lock = ReadWriteLock()

    def parse(self):
        id_to_quantity = {}
        for id, (_, quantity) in self.__products_to_quantities.items():
            id_to_quantity.update({id: quantity})
        return StoreData(self.__id, self.__name, id_to_quantity)

    def parse_products(self):
        parsed_products = []
        for product, quantity in self.__products_to_quantities.values():
    ...
    def get_products(self) -> Response[ParsableList[Product]]:
        self.__products_lock.acquire_read()
        products = [product for product, _ in self.__products_to_quantities.values()]
        self.__products_lock.release_read()
    ...
    def set_product_name(self, product_id, new_name) -> Response[None]:
        self.__products_lock.acquire_write()
        if self.__products_to_quantities[product_id] is None:
            self.__products_lock.release_write()
            return Response(False, msg=f"product with {product_id} doesn't exist in the store!")

        self.__products_to_quantities[product_id][0].set_product_name(new_name)
        self.__products_lock.release_write()
    ...
    def add_product(self, product_name: str, price: float, quantity: int) -> Response[str]:
        ...
        self.__products_to_quantities[product_id] = (product, quantity)
        self.__products_lock.release_write()
    ...
    def remove_product(self, product_id: str) -> Response[None]:
        self.__products_lock.acquire_write()
        result = self.__products_to_quantities.pop(product_id, None)
        if result is None:
            self.__products_lock.release_write()
            return Response(
    ...
    def edit_product_details(self, product_id: str, product_name: str, price: float) -> Response[None]:
        self.__products_lock.acquire_write()
        if product_id not in self.__products_to_quantities:
            self.__products_lock.release_write()
            return Response(False, msg="No such product in the store")

        response = self.__products_to_quantities[product_id][0].edit_product_details(product_name, price)
        self.__products_lock.release_write()
        return response
    ...
    def change_product_quantity(self, product_id: str, quantity: int) -> Response[None]:
        ...
        if product_id in self.__products_to_quantities:
            new_product_quantity = (self.__products_to_quantities[product_id][0], quantity)
            self.__products_to_quantities[product_id] = new_product_quantity
            prod_name = self.__products_to_quantities[product_id][0].get_name()
            self.__products_lock.release_write()
    ...
    def check_existing_product(self, product_name: str):
        for (prod, quantity) in self.__products_to_quantities.values():
    ...
    def get_product_name(self, product_id: str):
        self.__products_lock.acquire_read()
        name = self.__products_to_quantities.get(product_id)[0].get_name()
        self.__products_lock.release_read()
        return name
    ...
    def send_back(self, products_to_quantities: dict):
        ...
        for prod_id, (product, quantity) in products_to_quantities.items():
            if self.__products_to_quantities.get(prod_id) is None:
                self.__products_to_quantities.update({prod_id: (product, quantity)})
            else:
                self.__products_to_quantities[prod_id] = (self.__products_to_quantities[prod_id][0], self.__products_to_quantities[prod_id][1]+quantity)
        self.__products_lock.release_write()
    ...
    def check_available_products(self, products_to_quantities: dict) -> Response[None]:
        ...
        for prod_id, (_, quantity) in products_to_quantities.items():
            prod_to_current_quantity = self.__products_to_quantities.get(prod_id)
    ...
    def acquire_products(self, products_to_quantities: dict) -> None:
        for prod_id, (product, quantity) in products_to_quantities.items():
            current_quantity = self.__products_to_quantities.get(prod_id)[1]
            if current_quantity == quantity:
                self.__products_to_quantities.pop(prod_id)
            else:
                product = self.__products_to_quantities.get(prod_id)[0]
                self.__products_to_quantities[prod_id] = (product, current_quantity - quantity)
        self.__products_lock.release_write()
    ...
    def get_product(self, product_id: str):
        self.__products_lock.acquire_read()
        prod = self.__products_to_quantities.get(product_id)[0]
        self.__products_lock.release_read()
        return prod

    def product_exists(self, product_id,):
        self.__products_lock.acquire_read()
        product_quantity = self.__products_to_quantities.get(product_id)
    ...
    def has_enough(self, product_id, quantity):
        self.__products_lock.acquire_read()
        product_quantity = self.__products_to_quantities.get(product_id)
        if product_id in self.__products_to_quantities and product_quantity[1] < quantity:
            self.__products_lock.release_read()
            return False
        self.__products_lock.release_read()
        return True
