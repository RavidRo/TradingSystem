...
async def search_products():
    ...
    answer = await loop.run_in_executor(system.search_products, product_name, category, min_price, max_price, **kwargs)
    return json.dumps([ob.__dict__ for ob in answer.get_obj()])
...
async def save_product_in_cart():
    ...
    answer = await loop.run_in_executor(system.save_product_in_cart, cookie, store_id, product_id, quantity)
    return json.dumps({"cookie": cookie, "answer": answer.get_msg(), "succeeded": answer.succeeded()})
...
async def remove_product_from_cart():
    ...
    product_id = request.get_json()["product_id"]
    quantity = request.get_json()["quantity"]
    answer = await loop.run_in_executor(system.remove_product_from_cart, cookie, product_id, quantity)
    return json.dumps({"cookie": cookie, "answer": answer.get_msg(), "succeeded": answer.succeeded()})
...
async def change_product_quantity_in_cart():
    ...
    product_id = request.get_json()["product_id"]
    quantity = request.get_json()["quantity"]
    answer = await loop.run_in_executor(system.remove_product_from_cart, cookie, store_id, product_id, quantity)
    return json.dumps({"cookie": cookie, "answer": answer.get_msg(), "succeeded": answer.succeeded()})
...
async def create_product():
    ...
    quantity = request.get_json()["quantity"]
    answer = await loop.run_in_executor(system.create_product, cookie, store_id, name, price, quantity)
...
async def remove_products():
    ...
    store_id = request.get_json()["store_id"]
    product_id = request.get_json()["product_id"]
    answer = await loop.run_in_executor(system.remove_product_from_store, cookie, store_id, product_id)
    return json.dumps({"cookie": cookie, "answer": answer.get_msg(), "succeeded": answer.succeeded()})
...
async def change_product_quantity():
    ...
    product_id = request.get_json()["product_id"]
    quantity = request.get_json()["quantity"]
    answer = await loop.run_in_executor(system.change_product_quantity_in_store, cookie, store_id, product_id, quantity)
    return json.dumps({"cookie": cookie, "answer": answer.get_msg(), "succeeded": answer.succeeded()})
...
async def edit_product_details():
    ...
    answer = await loop.run_in_executor(system.edit_product_details, cookie, store_id, product_id, new_name, new_price)
    return json.dumps({"cookie": cookie, "answer": answer.get_msg(), "succeeded": answer.succeeded()})
...
async def add_manager_permission():
    ...
    answer = await loop.run_in_executor(system.add_manager_permission, cookie, store_id, username, permission_number)
    return json.dumps({"cookie": cookie, "answer": answer.get_msg(), "succeeded": answer.succeeded()})
...
async def remove_manager_permission():
    ...
    username = request.get_json()["username"]
    permission_number = request.get_json()["permission_number"]
    answer = await loop.run_in_executor(system.remove_manager_permission, cookie, store_id, username, permission_number)
    return json.dumps({"cookie": cookie, "answer": answer.get_msg(), "succeeded": answer.succeeded()})
